...
export class Page {
  ...
  constructor(private outputDir: string) {
    this.videoRecorder = new VideoRecorder(outputDir);
  }
  ...
  public async login(username: string, password: string): Promise<void> {
    await this.videoRecorder.startRecording(); // Start recording before actions
    await this.enterUsername(username);
    await this.enterPassword(password);
    await this.clickLoginButton();
    await browser.pause(10000); // Example pause for demonstration
    await this.videoRecorder.stopRecording(browser); // Stop recording after actions
  }
}
