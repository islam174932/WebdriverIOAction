...
export const config: Options.Testrunner = {
  ...
  // Hooks
    ...
    onPrepare: () => {
      // Get the directory path of the current file using import.meta.url
      ...
      if (fs.existsSync(allureResultsDir)) {
        // Remove the allure-results directory recursively and forcefully
        try {
          fs.rmSync(allureResultsDir, { recursive: true, force: true });
          console.log('Old Allure results removed.');
        } catch (err) {
          console.error('Error removing old Allure results:', err);
        }
      }
    },
