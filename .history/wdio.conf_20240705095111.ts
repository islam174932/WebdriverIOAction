...
export const config: Options.Testrunner = {
  ...
  afterStep: async function (step, scenario, result) {
    ...
    if (!result.passed) {
        console.log(`Step error: ${result.error ? result.error.message : 'No error message available'}`);
        
        // Take a screenshot if the step failed
        try {
            const screenshot = await browser.sa();
            const screenshotDir = path.join(__dirname, 'errorShots');
